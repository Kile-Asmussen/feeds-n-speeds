...
-- table.new tests
fact('table.new() creates empty table', function()
    local tbl = table.new()
    assert_is(tbl, 'table')
    assert_eq(#tbl, 0)
end)

fact('table.new(nil) creates empty table', function()
    local tbl = table.new(nil)
    assert_is(tbl, 'table')
    assert_eq(#tbl, 0)
end)

fact('table.new() wraps non-table in table', function()
    local tbl = table.new(42)
    assert_eq(tbl[1], 42)
    assert_eq(#tbl, 1)
end)

fact('table.new() with multiple args packs them', function()
    local tbl = table.new(1, 2, 3)
    assert_eq(tbl[1], 1)
    assert_eq(tbl[2], 2)
    assert_eq(tbl[3], 3)
    assert_eq(tbl.n, nil)
...
fact('table.new() preserves existing table', function()
    local orig = {a = 1, b = 2}
    local tbl = table.new(orig)
    assert_eq(tbl.a, 1)
    assert_eq(tbl.b, 2)
end)

fact('table.new() adds metatable with table methods', function()
    local tbl = table.new({1, 2, 3})
    assert_is(tbl.insert, 'function')
    assert_is(tbl.remove, 'function')
end)
