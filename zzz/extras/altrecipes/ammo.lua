...
function make_new_ammo_recipe(name, amount, ingredients)
    local ammo = table.clone(data.raw.recipe[name])
    ammo.name = fns(name .. '-mass-production')
    ammo.icon = (data.raw.ammo[name] or data.raw.capsule[name]).icon
    ...
    utilities.iconify(ammo, data.raw.item['explosives'].icon)
    return ammo
end
